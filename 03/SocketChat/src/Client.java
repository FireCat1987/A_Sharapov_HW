...
class Client {
        ...
        Runnable recieve = () -> {
            while (true) {
                try {
                    ...
                    ds.receive(packet);
                    ds.close();
                    String[] serverAnsver = new String(packet.getData(), 0, packet.getLength()).split("/");

                    System.out.println("Сервер найден, подключаюсь...");
                    client = new Socket(packet.getAddress().getHostAddress(),6666);
                    System.out.println("Поиск и подключение к серверу " + serverAnsver[0] +" выполнено успешно. На сервере находится " + serverAnsver[1] + " посетителей!");
                } catch (IOException e) {
    ...
    private void connect() {
        try {
            ...
            System.out.println("Сервер найден, подключаюсь...");
            client = new Socket(packet.getAddress().getHostAddress(),6666);
            System.out.println("Поиск и подключение к серверу выполнено успешно. На сервере находится " + serverAnsver[1] + " посетителей!");
        } catch (IOException e) {
            System.err.println("Ощибка при подключении к серверу!");
            if(stepConnect < 3) {
                connect();
